...
int main(int argc, char* argv[]) {
  ...
  std::vector<std::vector<int>> reg_id(max_y - min_y + 1,
                                       std::vector<int>(max_x - min_x + 1, 0));
  for (const auto& [coord, col] : TileIsWhite) {
    if (col == 1) {
      reg_id[coord.second - min_y][coord.first - min_x] = 1;
    }
  }
  for (int i = reg_id.size() - 1; i >= 0; i--) {
    for (const auto& ele : reg_id[i]) {
      if (ele == 1) {
        std::cout << 1 << ' ';
      } else {
        std::cout << ' ' << ' ';
