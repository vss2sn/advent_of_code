        ...
        if parts == 1:
            ...
            for i in range(0, len(message) - 2):
                for j in range(i + 1, len(message) - 1):
                    if (
                        DoesMsgFollowRule(message[0 : i + 1], subsection[0])
                        and DoesMsgFollowRule(message[i + 1 : j + 1], subsection[1])
                        and DoesMsgFollowRule(
                            message[j + 1 : len(message)], subsection[2]
                        )
                    ):
                        previously_seen_pairs[(message, source)] = True
                        return True
