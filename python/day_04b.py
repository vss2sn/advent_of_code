...
def main():
    ...
    rules = {"byr": lambda x : int(x) >= 1920 and int(x) <= 2002, \
             ...
             "hcl": lambda x : x[0] == '#' and all(char in '0123456789abcdef' for char in  x[1:]) , \
    ...
    for line in f:
        ...
        for field in fields:
            ...
            if split_field[0].strip() in key_fields and rules[split_field[0].strip()](split_field[1].strip()):
                n_fields += 1
    if(n_fields == len(key_fields)):
        n_valid += 1
    print(n_valid)
