...
int main(int argc, char* argv[]) {
  ...
  std::unordered_map<int, char> password;
  std::size_t i = 0;
  ...
  while(password.size() < 8) {
    ...
    const auto convert_to_hash_str = [](const uint32_t i) {
        std::stringstream stream;
        stream << std::hex << i << std::dec;
    ...
    if (hash_parts[0] <= 0x00000fff) {
        const auto result = convert_to_hash_str(hash_parts[0]);
        if(result[5] > '7') {
            i++;
            continue;
        }
        // for (const auto& hash : hash_parts) {
        //     std::cout << convert_to_hash_str(hash) << ' ';
        // }
        // std::cout << '\n';
        // std::cout << result << '\n';
        // if (result[5] <= '9' && result[5] >= '0') {
        if (password.find(result[5] - '0') == password.end()) {
            std::cout << "Index: " << result[5] << " Value: " << result[6] << '\n';
            password[result[5] - '0'] = result[6];
        }
    }
    i++;
  }
  std::string password_str  (8, '0');
  for (const auto [idx, ele] : password) {
    std::cout << idx << ": " << ele << '\n';
    password_str[idx] = ele;
  }
  std::cout << password_str << '\n';
  return 0;
}