...
#include <string>
#include <vector>
// TODO(vss): improve the previous_rounds so it can be generalized to n players.
// Rest already generalized. Multimap?
std::tuple<int, std::vector<std::deque<int>>, std::vector<int>>
PlayRecursiceCombat(std::vector<std::deque<int>> player_decks,
  ...
  while (player_decks.size() > 1) {
    if (auto it = previous_rounds.find(player_decks[0]);
        it != previous_rounds.end() &&
        it->second.find(player_decks[1]) != it->second.end()) {
    ...
    if (n_remaining_greater_than_card_value) {
      ...
      for (int i = 0; i < top_cards.size(); i++) {
        subgame_player_decks.emplace_back(
            std::begin(player_decks[i]),
            std::begin(player_decks[i]) + top_cards[i]);
      }
    ...
    auto dist = std::distance(
        std::begin(players),
        std::find(std::begin(players), std::end(players), winning_player));
    std::rotate(std::begin(top_cards), std::begin(top_cards) + dist,
                std::end(top_cards));

    if (std::find(std::begin(players), std::end(players), winning_player) ==
        std::end(players)) {
...
int main() {
  ...
  auto dist = std::distance(
      std::begin(std::get<2>(result)),
      std::find(std::begin(std::get<2>(result)), std::end(std::get<2>(result)),
                std::get<0>(result)));
  long long score = 0;
