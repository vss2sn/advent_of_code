...
std::vector<std::string> parse(const std::string& data,
  ...
  while (end != std::string::npos) {
    ...
    data_substr.erase(
        std::remove_if(std::begin(data_substr), std::end(data_substr),
...
int main() {
  ...
  while (std::getline(file, line)) {
    ...
    std::vector<std::string> allergens =
        parse(line.substr(con_ind + delimiter.size(),
                          line.size() - con_ind - delimiter.size()),
              ", )");
    ...
    for (const auto& allergen : allergens) {
      // For every allergen that has not been seen before add the ingredients
      // that could potentially contain it
      if (allergen_map.find(allergen) == allergen_map.end()) {
        ...
        // For every allergen that has been seen before remove the ingredients
        // that could potentially contain it
  ...
  std::unordered_map<std::string, std::string> known_allergic_ingredients;
  bool changed = true;
  while (changed) {
    changed = false;
    ...
    for (const auto& [allergen, potential_ingredients] : allergen_map) {
      if (potential_ingredients.size() == 1) {
        ...
        known_allergic_ingredients.insert(
            {ingredient_to_remove, allergen_to_remove});
        changed = true;
        break;
      }
    }
    ...
    for (auto& [allergen, potential_ingredients] : allergen_map) {
      if (potential_ingredients.find(ingredient_to_remove) !=
          potential_ingredients.end()) {
        potential_ingredients.erase(ingredient_to_remove);
      }
    }
  }
  long long count = 0;
  for (auto& [ingredient, ingredient_count] : ingredients_count) {
    if (known_allergic_ingredients.find(ingredient) ==
            known_allergic_ingredients.end() &&
        allergen_map.find(ingredient) == allergen_map.end()) {
