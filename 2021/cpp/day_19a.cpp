...
int main(int argc, char * argv[]) {
  ...
  std::unordered_set<Point, hash_point> all_beacons;
  for (auto& s : scanners) {
    const auto transform = transforms[s.chosen_orienation];
    for (const auto& p : s.input_beacons) {
      const auto tp = transform(p) + s.coordinates;
      s.beacon_coordinates_in_global_frame.push_back(tp);
      all_beacons.insert(tp);
    }
  }

  for (int i = 0; i < scanners.size(); i++) {
    // std::cout << scanners[i].coordinates << '\n';
    for (int j = i + 1; j < scanners.size(); j++) {
      // std::cout << "Common between " << i <<" and "<< j << '\n';
      for (const auto& coord : scanners[j].beacon_coordinates_in_global_frame) {
        if (const auto it = std::find(std::begin(scanners[i].beacon_coordinates_in_global_frame), std::end(scanners[i].beacon_coordinates_in_global_frame), coord);
            it != scanners[i].beacon_coordinates_in_global_frame.end()) {
          // std::cout << coord << '\n';
        }
      }
    }
