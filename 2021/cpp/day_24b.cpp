...
// Only change from part a is the initial number
...
std::vector<int> find_smallest_number(const std::vector<Instr>& instructions) {
  const auto n = instructions.size() / 14;
  auto digits = std::vector<int>(14, 1);
...
int main(int argc, char * argv[]) {
  ...
  const auto ans = find_smallest_number(instructions);
  print(ans);
  return 0;
}
