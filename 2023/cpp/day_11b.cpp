  ...
  for (int idx_1 = 0; idx_1 < galaxies.size(); idx_1++) {
    ...
    for (int idx_2 = idx_1+1; idx_2 < galaxies.size(); idx_2++) {
      ...
      total += std::abs(galaxies[idx_2].row - galaxies[idx_1].row) +
               ...
               std::abs(empty_rows_before_galaxy[idx_1] - empty_rows_before_galaxy[idx_2]) * multiplicative_factor +
               std::abs(empty_cols_before_galaxy[idx_1] - empty_cols_before_galaxy[idx_2]) * multiplicative_factor;
    }
  }
  std::cout << total << '\n';
  return 0;
