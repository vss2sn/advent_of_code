...
void target_selection(Army& a1, Army& a2) {
  ...
  selected_targets = std::unordered_set<int>();
...
bool attack(std::vector<Army>& armies, std::vector<InitStruct>& init_order) {
  bool change = false;
  for (auto& io : init_order) {
    ...
    if (std::find(std::begin(g2.immunities), std::end(g2.immunities), g1.attack_type) != g2.immunities.end()) {
      damage = 0;
    }
    change = change || (damage/g2.hp > 0);
...
int main(int argc, char* argv[]) {
  ...
  while(std::getline(file, line)) {
    ...
    if constexpr (debug) std::cout << group << '\n';
  ...
  const auto armies_main = armies;
  int boost = 0;
  bool found = false;
  while(!found) {
    bool loop = false;
    armies = armies_main;
    for (auto& g : armies[0].groups) {
    ...
    while (armies[0].is_alive() && armies[1].is_alive()) {
      ...
      for (auto& army : armies) {
        ...
        const auto sort_attacking_order = [](const auto& g1, const auto& g2) {
          return (g1.effective_power > g2.effective_power) || ((g1.effective_power == g2.effective_power) && (g1.initiative > g2.initiative));
        };
      ...
      target_selection(armies[0], armies[1]);
      if constexpr(debug) std::cout << '\n';
      bool changed = attack(armies, init_order);
      if constexpr(debug) std::cout << '\n';
      if (!changed) {
        loop = true;
        break;
    ...
    if (loop) {
      boost++;
    } else if (armies[immune_system_army_index].is_alive() && !armies[infection_army_index].is_alive()) {
      found = true;
      break;
    } else if (!armies[immune_system_army_index].is_alive() && armies[infection_army_index].is_alive()) {
      boost++;
    } else {
      if constexpr (debug) std::cout << "This should not happen" << '\n';
    }
  }
  if constexpr (debug) std::cout << boost << '\n';
  const auto ans = armies[immune_system_army_index].get_units();
  std::cout << ans << '\n';
  return ans;
}
