...
int main(int argc, char* argv[]) {
  ...
  const int n_players = stoi(parsed_input[0]);
  const int n_marbles = stoi(parsed_input[6]);
  ...
  const int max_score = *std::max_element(std::begin(scores), std::end(scores));
  std::cout << max_score << '\n';
  return max_score;
}
