...
int main(int argc, char * argv[]) {
    ...
    // exit(0);
    // for (const auto row :time_offsets) {
    //     std::cout << "|||";
    //     for (const auto ele : row) {
    //         for (const auto storm : ele) {
    //             std::cout << storm << ' ';
    //         }
    //         std::cout << "-";
    //     }
    ...
    // const auto distance = [&n_cols, &n_rows](const State& s) {
    //     return std::abs(n_rows-1 - s.row) + std::abs(n_cols - 2 - s.col);
    // };
    
    // const auto CompareStateGoalDist = [&distance](const State& s1, const State& s2) {
    //     return distance(s1) < distance(s2);
    // };

    struct CompareStateGoalDist {
        int distance (const State& s) const {
            return std::abs(s.row) + std::abs(s.col);
        };
        bool operator () (const State& s1, const State& s2) const {
            return distance(s1) < distance(s2);
        }
    };
    // std::priority_queue<State, std::vector<State>, CompareStateGoalDist> q;
    std::queue<State> q;
    const auto start = State(0,1,0);
    q.push(start);
    ...
    while(!q.empty()) {
        ...
        // if (current.time > 11) exit(0);
        // std::cout << current << '\n';
        // if (current.time > 18) {
        //     std::cout << "Nope" << '\n';
        //     exit(0);
        // }
        if (current.row == n_rows - 1 && current.col == n_cols - 2) {
