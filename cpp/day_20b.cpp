...
struct Tile {
  ...
  std::string& getRightEdge() {
    return edges[1];
  }

  std::string& getBottomEdge() {
    return edges[2];
  }
...
// the tile has to match the edge passed in along a particular orientation, ie, edge_dir
// so if the edge passed in is a right edge of the prev tile,
// the edge_dir should be 3, for the right edge of the current tile
// 0 = up, 1 = right, 2 = left and 3 = right
void OrientTileToMatchOnSide(const std::string& edge, Tile& cur_tile, const int edge_dir) {
  for(int i = 0; i < 4; ++i) {
...
bool backtrack(const int puzzle_size, const int count,
  ...
  if (col == 0) { // row and col is never 0 as set the tile for 0,0 before calling backtrack
    ...
    for(const auto id : ids) {
      ...
      OrientTileToMatchOnSide(bottom_edge, cur_tile, 0);
      const auto& right_edge = tiles[grid[row][col - 1]].getRightEdge();
      if(cur_tile.edges[3] == right_edge && backtrack(puzzle_size, count + 1, grid, edge_map, tiles)) {
        return true;
      } else {
        cur_tile.used = false;
        grid[row][col] = -1;
...
long long CheckForPattern(const std::vector<std::string>& complete_image, const std::vector<std::string>& pattern) {
...
int main() {
  ...
  std::vector<std::vector<int>> grid(puzzle_size, std::vector<int>(puzzle_size, -1));
  // Using the corners as starting points, though the algorithm will work starting form any tile
  for(const auto corner_id : corner_ids) {
    ...
    if (found) break;
    else tiles[corner_id].used = false;
  }

  // Create complete image and remove edges
  std::vector<std::string> complete_image((tiles[grid[0][0]].image.size() - 2) * (puzzle_size), "");
  for(int i = 0; i < puzzle_size; i++) {
    for(int j = 0; j < puzzle_size; j++) {
      ...
      for(int k = 0; k < (tile_img.size() - 2); k++) {
        complete_image[i * (tile_img.size() - 2) + k] += tile_img[k+1].substr(1, tile_img[k].size() - 2);
      }
    }
  }
