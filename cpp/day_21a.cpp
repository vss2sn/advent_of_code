...
int main() {
  ...
  while(changed) {
    ...
    for (const auto& [allergen, potential_ingredients] : allergen_map) {
      if(potential_ingredients.size() == 1) {
        ...
        known_allergic_ingredients.insert({ingredient_to_remove, allergen_to_remove});
        changed = true;
        break;
      }
    }
  ...
  for(auto& [ingredient, ingredient_count] : ingredients_count) {
    if (known_allergic_ingredients.find(ingredient) == known_allergic_ingredients.end() && allergen_map.find(ingredient) == allergen_map.end()) {
