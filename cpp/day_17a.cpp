...
int getActiveNeigbourCount(const std::vector<std::vector<std::string>>& volume,
  ...
  for (int d_plane = -1; d_plane < 2; d_plane++) {
    ...
    for (int d_row = -1; d_row < 2; d_row++) {
      ...
      for (int d_col = -1; d_col < 2; d_col++) {
        ...
        if (new_col < 0 || new_col >= volume[new_plane][new_row].size())
          continue;
        if (volume[new_plane][new_row][new_col] == '#' &&
            !(plane == new_plane && col == new_col && row == new_row))
          count++;
      }
    }
  }
...
int main() {
  ...
  std::vector<std::string> inactive_plane{g_plane.size(),
                                          std::string(g_plane[0].size(), '.')};
  ...
  std::vector<std::vector<std::vector<int>>> neighbour_count{
      volume.size(),
      std::vector<std::vector<int>>(volume[0].size(),
                                    std::vector<int>(volume[0][0].size(), 0))};
  for (int step = 0; step < n_steps; step++) {
    for (int plane = 0; plane < volume.size(); plane++) {
      for (int row = 0; row < volume[plane].size(); row++) {
        for (int col = 0; col < volume[plane][row].size(); col++) {
          neighbour_count[plane][row][col] =
              getActiveNeigbourCount(volume, plane, row, col);
        }
      }
    }
    ...
    for (int plane = 0; plane < volume.size(); plane++) {
      for (int row = 0; row < volume[plane].size(); row++) {
        for (int col = 0; col < volume[plane][row].size(); col++) {
          if (volume[plane][row][col] == '#' &&
              !(neighbour_count[plane][row][col] == 2 ||
                neighbour_count[plane][row][col] == 3)) {
            volume[plane][row][col] = '.';
          } else if (volume[plane][row][col] == '.' &&
                     neighbour_count[plane][row][col] == 3) {
            volume[plane][row][col] = '#';
          }
        }
  ...
  for (const auto& plane : volume) {
    for (const auto& line : plane) {
      count += std::count_if(std::begin(line), std::end(line),
