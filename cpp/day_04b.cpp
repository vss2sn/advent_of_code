...
int main() {
  ...
  std::unordered_map<std::string, bool (*)(const std::string&)> rules{
      {"byr",
       [](const std::string& val) {
         ...
         return i_val >= 1920 && i_val <= 2002;
       }},
      {"iyr",
       [](const std::string& val) {
         const int i_val = std::stoi(val);
         return i_val >= 2010 && i_val <= 2020;
       }},
      {"eyr",
       [](const std::string& val) {
         const int i_val = std::stoi(val);
         return i_val >= 2020 && i_val <= 2030;
       }},
      {"hgt",
       [](const std::string& val) {
      ...
      {"hcl",
       [](const std::string& val) {
         return val.size() == 7 && val[0] == '#' &&
                (val.substr(1, val.size())
                     .find_first_not_of("0123456789abcdef") ==
                 std::string::npos);
       }},
      {"ecl",
       [](const std::string& val) {
         const std::unordered_set<std::string> allowed_vals{
  ...
  while (std::getline(file, text)) {
    ...
    const std::string key = text.substr(start, pos_colon - start);
    const std::string val =
        text.substr(pos_colon + 1, text.size() - pos_colon - 1);
    if (key_fields.find(key) != key_fields.end() && rules[key](val)) {
      ++fields;
    }
    start = end + 1;
