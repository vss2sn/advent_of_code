...
int main() {
  ...
  while (round < 100) {
    ...
    for (int i = 0; i < n_pick_up; i++) {
      if (current_index + 1 < order.size()) {
        ...
        cups_picked_up.push_back(
            order[current_index + 1 + delta - order.size()]);
        order.erase(std::begin(order) + current_index + 1 + delta -
                    order.size());
        delta--;
      }
    }
    ...
    for (int i = 0; i < n_pick_up; i++) {
      if (insert_index + i + 1 < order.size()) {
        order.insert(std::begin(order) + insert_index + i + 1,
                     cups_picked_up[i]);
      } else {
    ...
    round++;
    current_index =
        std::distance(std::begin(order),
                      std::find(std::begin(order), std::end(order), cur_val)) +
        1;
  ...
  std::rotate(std::begin(order),
              std::find(std::begin(order), std::end(order), 1),
              std::end(order));
  std::string ans_str = "";
